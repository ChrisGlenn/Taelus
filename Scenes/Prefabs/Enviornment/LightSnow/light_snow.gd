...
@onready var LIGHTSNOW = $ParallaxBackground/ParallaxLayer/Sprite2D
...
func _ready():
	...
	LIGHTSNOW.modulate.a = 0 # hide the snow
...
func snow(clock):
	...
	if Globals.interior:
		LIGHTSNOW.visible = false # hide the snow
		$Droplets.visible = false # hide the droplets
	else:
		LIGHTSNOW.visible = true # show the snow
		if stage < 2: $Droplets.visible = true
	# scroll the snow background
	PARALLAX.scroll_offset.x -= 24 * clock
	PARALLAX.scroll_offset.y += 44 * clock
	# snow
	if stage == 0:
		# fade the snow in and once it is at full 'visiblity' then start to spawn the droplets
		if LIGHTSNOW.modulate.a < 1:
			LIGHTSNOW.modulate.a += 0.4 * clock # slowly fade in
			# PLAY SOUND (increment volume here as well)
		else:
			LIGHTSNOW.modulate.a = 1 # keep it from exceeding 1
		...
		if LIGHTSNOW.modulate.a > 0:
			LIGHTSNOW.modulate.a -= 0.4 * clock # slowly fade out
		else:
			LIGHTSNOW.modulate.a = 0 # stop it from going below zero
